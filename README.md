...
所有DSC实验均在NETZSCH DSC 204F1 Phoenix上完成，气氛为氮气，升温速率10 K/min，降温速率20 K/min；结晶度计算采用各标准聚合物的理论100%结晶焓值：PET为140 J/g，HDPE为293 J/g，PLA为93 J/g。对于混合样品或含填料样品，DSC仪器默认以样品总质量计算单位质量熔融焓（J/g），此时需先将熔融峰面积（J/g）乘以样品总质量得到总热量（J），再除以TGA分析得到的该组分在样品中的实际质量，得到该组分的真实单位质量熔融焓（J/g），最后除以该组分100%结晶的理论焓值，方可得到真实的结晶度。
...
- **熔融峰1**：峰值**112.0°C**，面积**7.678 J/g**，归属于**PBAT组分的熔融**（PBAT典型Tm约110–120°C）。对于该PBAT+PLA共混体系，仪器给出的单位质量熔融焓默认以样品总质量（2.74 mg）为基准，需按TGA或配方确定的各聚合物实际质量分数修正，方可得到PBAT与PLA各自真实的单位质量熔融焓及结晶度。
- **熔融峰2**：峰值**148.9°C**，面积仅**0.3113 J/g**，归属于**PLA组分的熔融**（PLA典型Tm约150–175°C）。峰面积极小（仅0.31 J/g），意味着PLA组分含量低或结晶度极低，与冷结晶放热量（–5.544 J/g）相抵后净熔融焓约为2.1 J/g，按仪器默认值估算PLA结晶度约2.3%（净），经实际质量分数修正后其真实结晶度仍极低。
### 3.2 TGA 分析
...
- **升温段熔融峰**：峰值**128.7°C**，峰面积**94.29 J/g**（熔融焓）。仪器直接标注结晶度 **32.18%**（以HDPE理论熔融焓293 J/g为基准：94.29/293 = 32.18%），但此值为仪器默认以样品总质量（3 mg）为基准的单位质量焓，未考虑CaCO₃填料稀释效应。按TGA分析结果，该样品含约35% CaCO₃，有效HDPE质量分数约65%。修正计算如下：
  - 总熔融热量 = 94.29 J/g × 3 mg = 282.87 mJ
  - HDPE实际质量 = 3 mg × 0.65 = 1.95 mg
  - 真实单位质量熔融焓 = 282.87 mJ / 1.95 mg = **145.1 J/g**
  - 真实结晶度 = 145.1 / 293 = **49.5%**
  修正后的结晶度更接近纯HDPE的真实结晶度，仪器直接值（32.18%）与修正值（49.5%）两种计算方式各有其参考意义。
...
| 可降解塑料袋2号 | HDPE+CaCO₃ 35%+OBE | — | 128.7 | 111.3 | 94.29 (仪器值) / 145.1 (修正值) | **32.18%** (仪器默认) / **49.5%** (TGA质量修正) |
...
| 可降解塑料袋2号 | PE（无C=O） | <418（前驱）/ 457.6（主） | 457.6 | 12.91 | — | 128.7 | 32.18% / 49.5% (修正) |
### 6.3 热稳定性与使用温度窗口
